...
- The homepage's `Projects / 06` link opens `#projects`, a CSS `:target` screen sliding over the dragon. Six cards sit in the grid; five link to a page (Hold Up, PixelPup, Vedro, Shipyard, Helm) and Evido does not. Those links work without JS; hash changes stop any active rampage, manage focus, and update `aria-expanded`. Escape closes the gallery. The overlay itself is the scroll container (sticky `.projects-head` keeps Back reachable); the homepage behind it remains a fixed stage.
...
- Project previews are self-contained: the PixelPup dog is an embedded copy of `pixelpup/assets/dog_labrador.png`; its landscape is CSS. Hold Up uses Apple’s official iPhone 17 landscape bezel, embedded on the homepage and stored at `holdup/assets/iphone-17-black-landscape.png` on the product page. Keep both copies synchronized. Vedro and Evido cards are `<img>` of each product page's own `assets/card-preview.svg`, so the card and the page never drift. Preview product colors stay fixed across the parent ink/paper themes.
