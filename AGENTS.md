...
- The homepage's `Projects / 06` link opens `#projects`, a CSS `:target` screen sliding over the dragon. Seven cards sit in the grid; six link to a page (Hold Up, PixelPup, PixelMew, Vedro, Shipyard, Helm) and Evido does not. Those links work without JS; hash changes stop any active rampage, manage focus, and update `aria-expanded`. Escape closes the gallery. The overlay itself is the scroll container (sticky `.projects-head` keeps Back reachable); the homepage behind it remains a fixed stage.
...
- Project previews are self-contained: the PixelPup dog is an embedded copy of `pixelpup/assets/dog_labrador.png`; its landscape is CSS. Hold Up uses Apple’s official iPhone 17 landscape bezel, embedded on the homepage and stored at `holdup/assets/iphone-17-black-landscape.png` on the product page. Keep both copies synchronized. Vedro, Evido, and PixelMew cards are `<img>` of each product page's own `assets/card-preview.svg`, so the card and the page never drift. PixelMew's card and its `assets/social-preview.png` are composed from the app's real 360 px face renders (`store/screen-cardboard-club-day-360.png` and `screen-nightlight-night-360.png` in the PixelMew repo), so re-compose them rather than retouching if the faces change. Preview product colors stay fixed across the parent ink/paper themes.
...
- `pixelmew/` is not authored here: it is the PixelMew repo's own page, generated there by `python3 tools/build_gallery.py` from `brand/page.html`, `brand/page.css`, and `brand/page.js`, and deployed as `index.html` plus `brand/`, `faces/` (120 real face renders, one per cat × theme × day/night), and `cats/` (the twenty portraits, downscaled to 312 px with nearest-neighbor from the 1254 px originals). Edit the PixelMew repo and re-copy; do not diverge here. Only the head (canonical, Open Graph, the local icon set), the logo `href`, and the two links that pointed at repo files (`watchface/README.md`) are rewritten for the site. Its interactive gallery is the page's own script: the twenty cat buttons ship `disabled` and the theme/lighting controls are `.js-only`, so without JS the default Black Shorthair / Cardboard Club / day face and all copy still render.
