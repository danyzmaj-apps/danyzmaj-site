...
- PixelPup, PixelMew, vedro, and Evido pages keep their own icon (paw / cat / route mark / E monogram) with the equivalent files under `pixelpup/assets/`, `pixelmew/assets/`, `vedro/assets/`, and `evido/assets/`. PixelMew's set is derived from the app's own `watchface/resources/drawables/launcher_icon.png` (60 × 60, nearest-neighbor upscaled onto its cream bed); regenerate it from that PNG if the launcher icon changes.
